...
class OldDataPointRetrievalViewSet(viewsets.ModelViewSet):
    serializer_class = DataPointSerializerDisplay
    permission_classes = (IsAuthenticated,)
    ...
    def get_queryset(self):
        ...
        return IntDataPoint.objects \
            .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value).all() \
            .union(FloatDataPoint.objects
                   .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value).all()).order_by('-time')
        ...
        try:
            ...
            if min_date is not None:
                if max_date is not None:
                    if max_values is not None:
                        points = IntDataPoint.objects \
                                     .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                             time__gte=min_date, time__lte=max_date).all() \
                                     .union(FloatDataPoint.objects
                                            .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                                    time__gte=min_date, time__lte=max_date).all()).order_by('-time')[
                                 :max_values]
                    else:
                        points = IntDataPoint.objects \
                            .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                            ...
                            .union(FloatDataPoint.objects
                                   .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                           time__gte=min_date, time__lte=max_date).all()).order_by('-time').all()
                else:
                    if max_values is not None:
                        points = IntDataPoint.objects \
                                     .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                             time__gte=min_date).all() \
                                     .union(FloatDataPoint.objects
                                            .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                                    time__gte=min_date).all()).order_by('-time')[:max_values]
                    else:
                        points = IntDataPoint.objects \
                            .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                    time__gte=min_date).all() \
                            .union(FloatDataPoint.objects
                                   .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                           time__gte=min_date).all()).order_by('-time').all()
            elif max_date is not None:
                if max_values is not None:
                    points = IntDataPoint.objects \
                                 .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                         time__lte=max_date).all() \
                                 .union(FloatDataPoint.objects
                                        .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                                time__lte=max_date).all()).order_by('-time')[:max_values]
                else:
                    points = IntDataPoint.objects \
                        .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                time__lte=max_date).all() \
                        .union(FloatDataPoint.objects
                               .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value,
                                       time__lte=max_date).all()).order_by('-time').all()
            else:
                if max_values is not None:
                    points = IntDataPoint.objects \
                                 .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value).all() \
                                 .union(FloatDataPoint.objects
                                        .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value)
                                        .all()).order_by('-time')[:max_values]
                else:
                    points = IntDataPoint.objects \
                        .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value).all() \
                        .union(FloatDataPoint.objects
                               .filter(user=user, indicator__origin=Indicator.OriginType.LAB.value)
                               .all()).order_by('-time').all()
